...
TABLES = [
    ...
    "dim_especialidade", "dim_unidade", "dim_farmaceutico",
    ...
    "fato_medico_snapshot", "fato_dispensacao_dia",
...
def main():
    ...
    cur.execute("SELECT sum(dispensacoes), sum(assinadas), sum(canceladas) FROM prescricao.fato_dispensacao_dia")
    log(f"dispensacoes total/assinadas/canceladas: {cur.fetchone()}")
    cur.execute(
