...
def main():
    ...
    log(f"cursor: {len(rows)} linhas; sum = {sum(r[4] for r in rows):,}; dias = {len(dias)}")
    ...
    log(f"DW antes (dia >= {dias[0]}): {antes:,}")
    ...
    log(f"DW depois (dia >= {dias[0]}): {depois:,}  (delta {depois - antes:+,})")
    dw.close()
